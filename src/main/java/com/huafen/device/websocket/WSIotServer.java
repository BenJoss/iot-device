package com.huafen.device.websocket;
...
public class WSIotServer {
    ...
    //  注：底下WebSocket是当前类名
    private static CopyOnWriteArraySet<WSIotServer> webSockets =new CopyOnWriteArraySet<>();
    // 用来存在线连接用户信息
    ...
    public void sendAllMessage(String message) {
        ...
        for(WSIotServer webSocket : webSockets) {
            try {
                if(webSocket.session.isOpen()) {
                    webSocket.session.getAsyncRemote().sendText(message);
                }
            } catch (Exception e) {
            	log.error("【 广播消息】失败:"+e.getMessage());
            }
        }
    }
    ...
    public void sendOneMessage(String userId, String message) {
        ...
        if (session != null&&session.isOpen()) {
            try {
            	...
            	log.error("【 单点消息】失败:"+e.getMessage());
            }
        }
    }
